...
def _parse(content):
    ...
    sections = (section.strip() for section in re.sub(r'(\w+.*\w+)\s+_{2,}', '\0\g<1>\0', content).split('\x00') if section.strip())
    ...
    match = re.search(r'(?P<name>\w+\s*\w+)\s*(?P<phone>\(\w+\)\s*(\w+)\-(\w+))\W+(?P<email>.*@.[^ ]*)\W+(?P<address>.*)', detail)
    if match:
...
def main():
    ...
    parser.add_argument('-i', '--infile', required=True, help='input file name of the PDF for the converter')
    parser.add_argument('-o', '--outfile', type=argparse.FileType('w'), default=sys.stdout, help='output file name to save JSON')
...
class PDF_TO_JSON_TestCase(unittest.TestCase):
    ...
    def test_headers_exist(self):
        headers = {'name', 'phone', 'email', 'address', 'Education', 'Leadership Experience', 'Professional Experience', 'Additional Projects', 'Skills & Interests'}
        content = _get_content(self.fname)
        data = _parse(content)
